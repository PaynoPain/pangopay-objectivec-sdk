...
-(void) getLoyaltyExchangesWithIdentifier:(NSNumber*) identifier
    ...
    NSString *action = [NSString stringWithFormat:@"loyalties/%@/gifts.json",identifier];
    [NXOAuth2Request performMethod:@"POST"
                        onResource:[self generateUrl:@"coupons/user_call"]
                   usingParameters:@{
                                     @"action":action,
                       ...
                       if(!error){
                           @try {
                               ...
                               if([[responseDictionary objectForKey:@"success"] boolValue]){
                                   ...
                                   @catch (NSException *exception) {
                                       NSLog(@"no se ha podido parsear la fidelity %@ por el error %@",data,exception);
                                   }
                                   
                                   if(successHandler) successHandler(exchangeableCoupons);
...
-(void) getLoyaltyWithCommerceIdentifier:(NSNumber*) identifier
    ...
    NSString *action = [NSString stringWithFormat:@"commerces/%@/loyalties.json",identifier];
    [NXOAuth2Request performMethod:@"POST"
                        onResource:[self generateUrl:@"coupons/user_call"]
                   usingParameters:@{
                                     @"action":action,
